...
<CardGroup cols={2}>
  <Card title="Inline Registration" icon="code">
    Recommended. Register the webhook during `messages/send` so it is ready before the
    task starts.
  </Card>
...
<CodeGroup>
  ...
  ```json tasks/pushNotification/get
  {
    "jsonrpc": "2.0",
...
<AccordionGroup>
  <Accordion title="Webhook not receiving notifications">
    ...
    ```json
    { "method": "tasks/pushNotification/get", "params": {"task_id": "task-123"} }
    ```
