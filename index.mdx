---
title: Welcome to Bindu
description: Build A2A-native agents with DID identity, gRPC sidecars, skills, payments, and production-ready infrastructure.
mode: custom
---

export const CodeTabs = ({ tabs, defaultTab }) => {
  const [activeTab, setActiveTab] = React.useState(defaultTab || tabs[0]?.id);
  const currentTab = tabs.find((tab) => tab.id === activeTab) || tabs[0];

  return (
    <div className="flex flex-col gap-5 w-full">
      <style
        dangerouslySetInnerHTML={{
          __html: `
            .code-tabs-shell {
              overflow: hidden;
              border-radius: 18px;
              border: 1px solid rgba(255,255,255,0.06);
              background: #111113;
              box-shadow:
                0 20px 40px rgba(0,0,0,0.22),
                0 1px 0 rgba(255,255,255,0.03) inset;
            }

            .code-tabs-header {
              display: flex;
              align-items: center;
              justify-content: space-between;
              gap: 16px;
              padding: 12px 16px;
              background: linear-gradient(
                to bottom,
                rgba(255,255,255,0.05),
                rgba(255,255,255,0.02)
              );
              border-bottom: 1px solid rgba(255,255,255,0.05);
            }

            .code-tabs-topbar {
              display: flex;
              align-items: center;
              gap: 8px;
              min-width: 72px;
            }

            .code-tabs-dot {
              width: 11px;
              height: 11px;
              border-radius: 999px;
              box-shadow: inset 0 1px 1px rgba(255,255,255,0.25);
            }

            .code-tabs-dot.red {
              background: #ff5f57;
            }

            .code-tabs-dot.yellow {
              background: #febc2e;
            }

            .code-tabs-dot.green {
              background: #28c840;
            }

            .code-tabs-nav {
              display: flex;
              align-items: center;
              gap: 6px;
              flex-wrap: wrap;
              justify-content: center;
              flex: 1;
            }

            .code-tab-btn {
              border: none;
              background: transparent;
              color: rgba(255,255,255,0.42);
              padding: 7px 12px;
              border-radius: 8px;
              cursor: pointer;
              font-size: 14px;
              font-weight: 500;
              transition: all 0.2s ease;
            }

            .code-tab-btn:hover {
              color: rgba(255,255,255,0.75);
              background: rgba(255,255,255,0.04);
            }

            .code-tab-btn.active {
              color: #ffffff;
              background: rgba(255,255,255,0.08);
              box-shadow:
                inset 0 1px 0 rgba(255,255,255,0.05),
                0 4px 10px rgba(0,0,0,0.18);
            }

            .code-lang {
              min-width: 72px;
              text-align: right;
              color: rgba(255,255,255,0.28);
              font-size: 11px;
              font-weight: 600;
              letter-spacing: 0.16em;
              text-transform: uppercase;
            }

            .code-box {
              position: relative;
              padding: 24px;
              overflow-x: auto;
            }

            .code-box::before {
              content: "";
              position: absolute;
              inset: 0;
              background-image:
                linear-gradient(rgba(255,255,255,0.018) 1px, transparent 1px),
                linear-gradient(90deg, rgba(255,255,255,0.018) 1px, transparent 1px);
              background-size: 28px 28px;
              pointer-events: none;
            }

            .code-box pre {
              margin: 0;
              position: relative;
              z-index: 1;
              font-size: 13px;
              line-height: 1.8;
              font-family: "JetBrains Mono", "Fira Code", Monaco, Consolas, monospace;
              color: #e4e4e7;
            }

            .code-box code {
              white-space: pre;
            }

            @media (max-width: 768px) {
              .code-tabs-header {
                flex-direction: column;
                align-items: flex-start;
              }

              .code-tabs-nav {
                width: 100%;
                justify-content: flex-start;
              }

              .code-lang {
                display: none;
              }
            }
          `,
        }}
      />

      <div className="code-tabs-shell">
        <div className="code-tabs-header">
          <div className="code-tabs-topbar">
            <span className="code-tabs-dot red" />
            <span className="code-tabs-dot yellow" />
            <span className="code-tabs-dot green" />
          </div>

          <div className="code-tabs-nav">
            {tabs.map((tab) => {
              const isActive = activeTab === tab.id;

              return (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={isActive ? "code-tab-btn active" : "code-tab-btn"}
                >
                  {tab.label}
                </button>
              );
            })}
          </div>

          <span className="code-lang">
            {currentTab?.language || "code"}
          </span>
        </div>

        <div className="code-box">
          <pre>
            <code>{currentTab?.code}</code>
          </pre>
        </div>
      </div>
    </div>
  );
};

export const codeTabs = [
  {
    id: "python",
    label: "Basic Agent",
    language: "python",
    code: (
      <>
        <span style={{ color: "#8B5CF6" }}>from</span> bindu.penguin.bindufy{" "}
        <span style={{ color: "#8B5CF6" }}>import</span> bindufy{"\n"}
        {"\n"}
        config = {"{"}{"\n"}
        {"    "}
        <span style={{ color: "#c3e88d" }}>"author"</span>:{" "}
        <span style={{ color: "#c3e88d" }}>"dev@example.com"</span>,{"\n"}
        {"    "}
        <span style={{ color: "#c3e88d" }}>"name"</span>:{" "}
        <span style={{ color: "#c3e88d" }}>"hello-bindu"</span>,{"\n"}
        {"    "}
        <span style={{ color: "#c3e88d" }}>"description"</span>:{" "}
        <span style={{ color: "#c3e88d" }}>"A simple Bindu agent"</span>,{"\n"}
        {"    "}
        <span style={{ color: "#c3e88d" }}>"deployment"</span>: {"{"}
        <span style={{ color: "#c3e88d" }}>"url"</span>:{" "}
        <span style={{ color: "#c3e88d" }}>"http://localhost:3773"</span>,{" "}
        <span style={{ color: "#c3e88d" }}>"expose"</span>:{" "}
        <span style={{ color: "#8B5CF6" }}>True</span>
        {"}"},{"\n"}
        {"}"}{"\n"}
        {"\n"}
        <span style={{ color: "#8B5CF6" }}>def</span> handler(messages):{"\n"}
        {"    "}
        <span style={{ color: "#8B5CF6" }}>return</span>{" "}
        <span style={{ color: "#c3e88d" }}>"Hello from Bindu!"</span>
        {"\n"}
        {"\n"}
        bindufy(config, handler)
      </>
    ),
  },
  {
    id: "typescript",
    label: "TypeScript SDK",
    language: "typescript",
    code: (
      <>
        <span style={{ color: "#8B5CF6" }}>import</span> {"{"} bindufy {"}"}{" "}
        <span style={{ color: "#8B5CF6" }}>from</span>{" "}
        <span style={{ color: "#c3e88d" }}>"@getbindu/sdk"</span>;{"\n"}
        {"\n"}
        <span style={{ color: "#8B5CF6" }}>const</span> config = {"{"}{"\n"}
        {"  "}author: <span style={{ color: "#c3e88d" }}>"dev@example.com"</span>,{"\n"}
        {"  "}name: <span style={{ color: "#c3e88d" }}>"ts-bindu-agent"</span>,{"\n"}
        {"  "}description:{" "}
        <span style={{ color: "#c3e88d" }}>"A TypeScript Bindu agent"</span>,{"\n"}
        {"  "}deployment: {"{"}{"\n"}
        {"    "}url: <span style={{ color: "#c3e88d" }}>"http://localhost:3773"</span>,{"\n"}
        {"    "}expose: <span style={{ color: "#8B5CF6" }}>true</span>,{"\n"}
        {"  "}{"}"},{"\n"}
        {"}"};{"\n"}
        {"\n"}
        <span style={{ color: "#8B5CF6" }}>const</span> handler ={" "}
        <span style={{ color: "#8B5CF6" }}>async</span> (messages) =&gt; {"{"}{"\n"}
        {"  "}
        <span style={{ color: "#8B5CF6" }}>return</span>{" "}
        <span style={{ color: "#c3e88d" }}>"Hello from TypeScript and Bindu!"</span>;{"\n"}
        {"}"};{"\n"}
        {"\n"}
        <span style={{ color: "#8B5CF6" }}>await</span> bindufy(config, handler);
      </>
    ),
  },
  {
    id: "kotlin",
    label: "Kotlin Agent",
    language: "kotlin",
    code: (
      <>
        <span style={{ color: "#ff79c6" }}>import</span> com.getbindu.sdk.ChatMessage{"\n"}
        <span style={{ color: "#ff79c6" }}>import</span> com.getbindu.sdk.bindufy{"\n"}
        <span style={{ color: "#ff79c6" }}>import</span> java.net.URI{"\n"}
        <span style={{ color: "#ff79c6" }}>import</span> java.net.http.HttpClient{"\n"}
        <span style={{ color: "#ff79c6" }}>import</span> java.net.http.HttpRequest{"\n"}
        <span style={{ color: "#ff79c6" }}>import</span> java.net.http.HttpResponse{"\n"}
        <span style={{ color: "#ff79c6" }}>import</span> com.google.gson.Gson{"\n"}
        <span style={{ color: "#ff79c6" }}>import</span> com.google.gson.JsonObject{"\n\n"}
        
        <span style={{ color: "#ff79c6" }}>val</span> httpClient: HttpClient = HttpClient.newHttpClient(){"\n"}
        <span style={{ color: "#ff79c6" }}>val</span> gson = Gson(){"\n\n"}
        
        <span style={{ color: "#ff79c6" }}>suspend fun</span> callOpenAI(messages: List{"<"}ChatMessage{">"}): String {"{"}{"\n"}
        {"    "}<span style={{ color: "#ff79c6" }}>val</span> apiKey = System.getenv(<span style={{ color: "#c3e88d" }}>"OPENAI_API_KEY"</span>){"\n"}
        {"        "}?: <span style={{ color: "#ff79c6" }}>throw</span> RuntimeException(<span style={{ color: "#c3e88d" }}>"OPENAI_API_KEY not set"</span>){"\n\n"}
        
        {"    "}<span style={{ color: "#ff79c6" }}>val</span> messagesJson = messages.map {"{"} msg -{">"}{"\n"}
        {"        "}mapOf(<span style={{ color: "#c3e88d" }}>"role"</span> to msg.role, <span style={{ color: "#c3e88d" }}>"content"</span> to msg.content){"\n"}
        {"    "}{"}"}{"\n\n"}
        
        {"    "}<span style={{ color: "#ff79c6" }}>val</span> body = gson.toJson(mapOf({" \n"}
        {"        "}<span style={{ color: "#c3e88d" }}>"model"</span> to <span style={{ color: "#c3e88d" }}>"gpt-4o"</span>,{"\n"}
        {"        "}<span style={{ color: "#c3e88d" }}>"messages"</span> to messagesJson{"\n"}
        {"    "})){"\n\n"}
        
        {"    "}<span style={{ color: "#ff79c6" }}>val</span> request = HttpRequest.newBuilder(){"\n"}
        {"        "}.uri(URI.create(<span style={{ color: "#c3e88d" }}>"https://api.openai.com/v1/chat/completions"</span>)){"\n"}
        {"        "}.header(<span style={{ color: "#c3e88d" }}>"Content-Type"</span>, <span style={{ color: "#c3e88d" }}>"application/json"</span>){"\n"}
        {"        "}.header(<span style={{ color: "#c3e88d" }}>"Authorization"</span>, <span style={{ color: "#c3e88d" }}>"Bearer $apiKey"</span>){"\n"}
        {"        "}.POST(HttpRequest.BodyPublishers.ofString(body)){"\n"}
        {"        "}.build(){"\n\n"}
        
        {"    "}<span style={{ color: "#ff79c6" }}>val</span> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString()){"\n"}
        {"    "}<span style={{ color: "#ff79c6" }}>val</span> json = gson.fromJson(response.body(), JsonObject::class.java){"\n\n"}
        
        {"    "}<span style={{ color: "#ff79c6" }}>return</span> json{"\n"}
        {"        "}.getAsJsonArray(<span style={{ color: "#c3e88d" }}>"choices"</span>){"\n"}
        {"        "}.get(0).asJsonObject{"\n"}
        {"        "}.getAsJsonObject(<span style={{ color: "#c3e88d" }}>"message"</span>){"\n"}
        {"        "}.get(<span style={{ color: "#c3e88d" }}>"content"</span>).asString{"\n"}
        {"}"}{"\n\n"}
        
        <span style={{ color: "#ff79c6" }}>fun</span> main() {"{"}{"\n"}
        {"    "}bindufy({" \n"}
        {"        "}config = mapOf({" \n"}
        {"            "}<span style={{ color: "#c3e88d" }}>"author"</span> to <span style={{ color: "#c3e88d" }}>"dev@example.com"</span>,{"\n"}
        {"            "}<span style={{ color: "#c3e88d" }}>"name"</span> to <span style={{ color: "#c3e88d" }}>"kotlin-openai-agent"</span>,{"\n"}
        {"            "}<span style={{ color: "#c3e88d" }}>"description"</span> to <span style={{ color: "#c3e88d" }}>"An assistant built with Kotlin and Bindu"</span>,{"\n"}
        {"            "}<span style={{ color: "#c3e88d" }}>"version"</span> to <span style={{ color: "#c3e88d" }}>"1.0.0"</span>,{"\n"}
        {"            "}<span style={{ color: "#c3e88d" }}>"deployment"</span> to mapOf({" \n"}
        {"                "}<span style={{ color: "#c3e88d" }}>"url"</span> to <span style={{ color: "#c3e88d" }}>"http://localhost:3773"</span>,{"\n"}
        {"                "}<span style={{ color: "#c3e88d" }}>"expose"</span> to <span style={{ color: "#ff79c6" }}>true</span>,{"\n"}
        {"                "}<span style={{ color: "#c3e88d" }}>"cors_origins"</span> to listOf(<span style={{ color: "#c3e88d" }}>"http://localhost:5173"</span>){"\n"}
        {"            "}),{"\n"}
        {"        "}),{"\n"}
        {"        "}skills = listOf(<span style={{ color: "#c3e88d" }}>"skills/kotlin-openai-skill"</span>){"\n"}
        {"    "}) {"{"} messages -{">"}{"\n"}
        {"        "}<span style={{ color: "#71717a" }}>// Call OpenAI and return the response</span>{"\n"}
        {"        "}callOpenAI(messages){"\n"}
        {"    "}{"}"}{"\n"}
        {"}"}
      </>
    ),
  }
];

<style>{`
  @keyframes drift-grid {
    0% {
      background-position: 0px 0px, 0px 0px;
    }
    100% {
      background-position: 80px 80px, 80px 80px;
    }
  }
`}</style>
<div className="relative mx-[calc(50%-50vw)] bg-[#f6f7f9] dark:bg-[#111113] py-16 md:py-16 border-y border-[#e5e7eb] dark:border-zinc-800 overflow-hidden">

<div
  className="absolute inset-0 pointer-events-none opacity-[0.55] dark:opacity-[0.18]"
  style={{
    backgroundImage: `
      radial-gradient(circle at center, rgba(24,24,27,0.035) 1px, transparent 1px),
      linear-gradient(to right, rgba(24,24,27,0.025) 1px, transparent 1px),
      linear-gradient(to bottom, rgba(24,24,27,0.025) 1px, transparent 1px)
    `,
    backgroundSize: "24px 24px, 96px 96px, 96px 96px",
    backgroundPosition: "center center",
    animation: "drift-grid 18s linear infinite",
    maskImage:
      "radial-gradient(circle at center, black 45%, transparent 95%)",
    WebkitMaskImage:
      "radial-gradient(circle at center, black 45%, transparent 95%)",
  }}
/>

  <div className="relative mx-auto flex max-w-6xl flex-col px-5 sm:px-8 lg:px-10">
    <header className="flex flex-col gap-3 rounded-full border border-[#e5e7eb] dark:border-zinc-700 bg-white dark:bg-zinc-900 px-5 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-3">
        <span className="text-lg font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">
          Bindu
        </span>
        <span className="rounded-full border border-[#e5e7eb] dark:border-zinc-700 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">
          Docs
        </span>
      </div>
      <div className="flex items-center gap-3">
        <a
          href="/bindu/introduction/what-is-bindu"
          className="rounded-full border border-[#d1d5db] dark:border-zinc-600 bg-white dark:bg-zinc-800 px-4 py-2 text-sm font-semibold text-zinc-800 dark:text-zinc-100 transition hover:bg-zinc-50 dark:hover:bg-zinc-700"
        >
          Read the docs
        </a>
        <a
          href="https://github.com/getbindu/bindu"
          className="rounded-full border border-[#8B5CF6] bg-[#8B5CF6] px-4 py-2 text-sm font-semibold text-white shadow-[0_10px_20px_rgba(139,92,246,0.25)] transition hover:bg-[#7C3AED]"
        >
          View on GitHub
        </a>
      </div>
    </header>

    <section className="mt-20 flex flex-col items-center gap-6 text-center">
      <style dangerouslySetInnerHTML={{__html: `
        @import url('https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,600&family=Space+Grotesk:wght@600&display=swap');

        @keyframes hero-blur-reveal {
          0% {
            opacity: 0;
            transform: translateY(16px);
            filter: blur(8px);
          }
          100% {
            opacity: 1;
            transform: translateY(0);
            filter: blur(0);
          }
        }

        .mac-hero-heading {
          /* Prioritizes Bricolage Grotesque for that premium agency feel */
          font-family: "Bricolage Grotesque", "Space Grotesk", "Geist", -apple-system, BlinkMacSystemFont, "SF Pro Display", sans-serif;
          letter-spacing: -0.03em;
        }

        .hero-line-1 {
          display: inline-block;
          /* Custom cubic-bezier for that signature Apple ease-out */
          animation: hero-blur-reveal 1s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }

        .hero-line-2 {
          display: inline-block;
          /* 150ms delay creates a beautiful staggered reading rhythm */
          animation: hero-blur-reveal 1s cubic-bezier(0.16, 1, 0.3, 1) 0.15s forwards;
          opacity: 0; /* Keeps it hidden before the animation starts */
        }

        .hero-highlight {
          /* Sleek, metallic-style text gradient instead of a glowing shadow */
          background: linear-gradient(135deg, #8B5CF6 0%, #a78bfa 100%);
          -webkit-background-clip: text;
          -webkit-text-fill-color: transparent;
          background-clip: text;
        }

        .dark .hero-highlight {
          background: linear-gradient(135deg, #a78bfa 0%, #d8b4fe 100%);
          -webkit-background-clip: text;
          -webkit-text-fill-color: transparent;
          background-clip: text;
        }
      `}} />

      <h1 className="mac-hero-heading text-4xl font-semibold leading-[1.05] text-zinc-800 dark:text-zinc-200 sm:text-5xl md:text-[68px]">
        <span className="hero-line-1">
          The Open-Source Framework
        </span>
        <br />
        <span className="hero-line-2 hero-highlight">
          for the Internet of Agents
        </span>
      </h1>

      <p className="max-w-3xl text-base leading-relaxed text-zinc-600 dark:text-zinc-300 sm:text-lg">
        Build agents that speak A2A, carry DID-native identity, expose
        production-ready infrastructure, and still let you write the logic in
        the language your team prefers.
      </p>

      <div className="mt-12 grid w-full max-w-5xl grid-cols-1 gap-4 md:grid-cols-3">
        <a
          href="/bindu/introduction/what-is-bindu"
          className="group relative flex flex-col overflow-hidden rounded-2xl border border-zinc-300/90 bg-white/90 p-0 shadow-[0_10px_30px_rgba(0,0,0,0.06)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-zinc-300 hover:shadow-[0_18px_50px_rgba(0,0,0,0.10)] dark:border-white/10 dark:bg-[#111113]/95 dark:shadow-[0_18px_50px_rgba(0,0,0,0.30)] dark:hover:border-zinc-600"
        >
          <div className="flex items-center justify-between border-b border-zinc-200/70 bg-gradient-to-b from-zinc-100 to-zinc-50 px-4 py-3 dark:border-white/5 dark:from-white/[0.05] dark:to-white/[0.02]">
            <span className="text-[11px] font-medium tracking-wide text-zinc-500 dark:text-zinc-400">
              Introduction
            </span>

            <div className="w-[52px]" />
          </div>

          {/* Soft glare sweep */}
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
            <div className="absolute inset-y-0 left-[-30%] w-[35%] rotate-[12deg] bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-0 transition-all duration-1000 group-hover:left-[120%] group-hover:opacity-100 dark:via-white/10" />
          </div>

          {/* Content */}
          <div className="relative z-10 flex flex-col gap-3 p-5">
            <div className="flex items-center gap-3">
              <div>
                <h3 className="text-[15px] font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">
                  Get Started
                </h3>
                <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
                  Learn the core concepts and build your first A2A protocol agent.
                </p>
              </div>
            </div>
          </div>
        </a>

        <a
          href="/bindu/grpc/overview"
          className="group relative flex flex-col overflow-hidden rounded-2xl border border-zinc-300/90 bg-white/90 p-0 shadow-[0_10px_30px_rgba(0,0,0,0.06)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-zinc-300 hover:shadow-[0_18px_50px_rgba(0,0,0,0.10)] dark:border-white/10 dark:bg-[#111113]/95 dark:shadow-[0_18px_50px_rgba(0,0,0,0.30)] dark:hover:border-zinc-600"
        >
          <div className="flex items-center justify-between border-b border-zinc-200/70 bg-gradient-to-b from-zinc-100 to-zinc-50 px-4 py-3 dark:border-white/5 dark:from-white/[0.05] dark:to-white/[0.02]">
            <span className="text-[11px] font-medium tracking-wide text-zinc-500 dark:text-zinc-400">
              SDKs
            </span>

            <div className="w-[52px]" />
          </div>

          {/* Soft glare sweep */}
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
            <div className="absolute inset-y-0 left-[-30%] w-[35%] rotate-[12deg] bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-0 transition-all duration-1000 group-hover:left-[120%] group-hover:opacity-100 dark:via-white/10" />
          </div>

          {/* Content */}
          <div className="relative z-10 flex flex-col gap-3 p-5">
            <div className="flex items-center gap-3">
              <div>
                <h3 className="text-[15px] font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">
                  The Sidecar Architecture
                </h3>
                <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
                  Write agent logic in any language while the Python core handles infrastructure.
                </p>
              </div>
            </div>
          </div>
        </a>

        <a
          href="/examples/beginner/echo-agent"
          className="group relative flex flex-col overflow-hidden rounded-2xl border border-zinc-300/90 bg-white/90 p-0 shadow-[0_10px_30px_rgba(0,0,0,0.06)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-zinc-300 hover:shadow-[0_18px_50px_rgba(0,0,0,0.10)] dark:border-white/10 dark:bg-[#111113]/95 dark:shadow-[0_18px_50px_rgba(0,0,0,0.30)] dark:hover:border-zinc-600"
        >
          <div className="flex items-center justify-between border-b border-zinc-200/70 bg-gradient-to-b from-zinc-100 to-zinc-50 px-4 py-3 dark:border-white/5 dark:from-white/[0.05] dark:to-white/[0.02]">
            <span className="text-[11px] font-medium tracking-wide text-zinc-500 dark:text-zinc-400">
              Agent Examples
            </span>

            <div className="w-[52px]" />
          </div>

          {/* Soft glare sweep */}
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
            <div className="absolute inset-y-0 left-[-30%] w-[35%] rotate-[12deg] bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-0 transition-all duration-1000 group-hover:left-[120%] group-hover:opacity-100 dark:via-white/10" />
          </div>

          {/* Content */}
          <div className="relative z-10 flex flex-col gap-3 p-5">
            <div className="flex items-center gap-3">
              <div>
                <h3 className="text-[15px] font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">
                  Examples
                </h3>
                <p className="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
                  Explore production patterns, from basic agents to multi-agent swarms.
                </p>
              </div>
            </div>
          </div>
        </a>
      </div>
    </section>

    <section className="mt-20 flex flex-col gap-6">
      <div className="flex flex-col gap-2 text-center">
        <p className="text-xs font-semibold uppercase tracking-[0.35em] text-zinc-500 dark:text-zinc-400">
          Build with bindu
        </p>
      </div>
      <div className="lg:p-10">
        <CodeTabs tabs={codeTabs} defaultTab="python" />
      </div>
    </section>

    <section className="mt-20 pb-12">
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-8 px-5 sm:px-8 lg:grid-cols-5 lg:px-10">
        <div>
          <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-zinc-900 dark:text-zinc-100">
            Get Started
          </h3>
          <ul className="space-y-3">
            <li>
              <a
                href="/bindu/introduction/what-is-bindu"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                What is Bindu?
              </a>
            </li>
            <li>
              <a
                href="/bindu/introduction/key-concepts"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                Key Concepts
              </a>
            </li>
            <li>
              <a
                href="/bindu/how-to/install"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                Install
              </a>
            </li>
          </ul>
        </div>

        <div>
          <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-zinc-900 dark:text-zinc-100">
            Architecture
          </h3>
          <ul className="space-y-3">
            <li>
              <a
                href="/bindu/concepts/protocol"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                The Protocol
              </a>
            </li>
            <li>
              <a
                href="/bindu/concepts/task-first-pattern"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                Task-First Pattern
              </a>
            </li>
            <li>
              <a
                href="/bindu/grpc/overview"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                gRPC Overview
              </a>
            </li>
          </ul>
        </div>

        <div>
          <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-zinc-900 dark:text-zinc-100">
            Features
          </h3>
          <ul className="space-y-3">
            <li>
              <a
                href="/bindu/learn/did/overview"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                DID Identity
              </a>
            </li>
            <li>
              <a
                href="/bindu/learn/payment/introduction"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                x402 Payments
              </a>
            </li>
            <li>
              <a
                href="/bindu/learn/storage/overview"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                Storage
              </a>
            </li>
            <li>
              <a
                href="/bindu/learn/scheduler/overview"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                Scheduler
              </a>
            </li>
          </ul>
        </div>

        <div>
          <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-zinc-900 dark:text-zinc-100">
            Reference
          </h3>
          <ul className="space-y-3">
            <li>
              <a
                href="/api-reference/authentication/oauth2-token-endpoint"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                API Docs
              </a>
            </li>
            <li>
              <a
                href="/examples/beginner/echo-agent"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                Examples
              </a>
            </li>
          </ul>
        </div>

        <div>
          <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-zinc-900 dark:text-zinc-100">
            Community
          </h3>
          <ul className="space-y-3">
            <li>
              <a
                href="https://github.com/getbindu/bindu"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                GitHub
              </a>
            </li>
            <li>
              <a
                href="https://getbindu.com"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                Website
              </a>
            </li>
            <li>
              <a
                href="https://discord.com/invite/3w5zuYUuwt"
                className="text-sm text-zinc-500 dark:text-zinc-400 transition hover:text-[#8B5CF6]"
              >
                Discord
              </a>
            </li>
          </ul>
        </div>
      </div>

      <div className="mx-auto mt-12 flex max-w-6xl items-center justify-between px-5 pt-8 sm:px-8 lg:px-10">
        <p className="text-xs text-zinc-400 dark:text-zinc-500">
          {"\u00A9"} 2025 Bindu. All rights reserved.
        </p>
        <div className="flex items-center gap-4">
          <a
            href="https://x.com/raahulrahl"
            className="text-zinc-400 dark:text-zinc-500 transition hover:text-[#8B5CF6]"
          >
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
          </a>
          <a
            href="https://github.com/getbindu"
            className="text-zinc-400 dark:text-zinc-500 transition hover:text-[#8B5CF6]"
          >
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
            </svg>
          </a>
          <a
            href="https://discord.com/invite/3w5zuYUuwt"
            className="text-zinc-400 dark:text-zinc-500 transition hover:text-[#8B5CF6]"
          >
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M20.317 4.37a19.791 19.791 0 00-4.885-1.515.074.074 0 00-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 00-5.487 0 12.64 12.64 0 00-.617-1.25.077.077 0 00-.079-.037A19.736 19.736 0 003.677 4.37a.07.07 0 00-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 00.031.057 19.9 19.9 0 005.993 3.03.078.078 0 00.084-.028 14.09 14.09 0 001.226-1.994.076.076 0 00-.041-.106 13.107 13.107 0 01-1.872-.892.077.077 0 01-.008-.128 10.2 10.2 0 00.372-.292.074.074 0 01.077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 01.078.01c.12.098.246.198.373.292a.077.077 0 01-.006.127 12.299 12.299 0 01-1.873.892.077.077 0 00-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 00.084.028 19.839 19.839 0 006.002-3.03.077.077 0 00.032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 00-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
            </svg>
          </a>
          <a
            href="https://getbindu.com"
            className="text-zinc-400 dark:text-zinc-500 transition hover:text-[#8B5CF6]"
          >
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2a10 10 0 100 20 10 10 0 000-20zm6.93 9h-3.16a15.87 15.87 0 00-1.15-4.31A8.03 8.03 0 0118.93 11zM12 4.07c.89 1.08 1.97 3.14 2.52 6.93H9.48C10.03 7.21 11.11 5.15 12 4.07zM4.07 13h3.16c.17 1.59.55 3.07 1.15 4.31A8.03 8.03 0 014.07 13zm3.16-2H4.07a8.03 8.03 0 014.31-4.31A15.87 15.87 0 007.23 11zm4.77 8.93c-.89-1.08-1.97-3.14-2.52-6.93h5.04c-.55 3.79-1.63 5.85-2.52 6.93zM15.62 17.31c.6-1.24.98-2.72 1.15-4.31h3.16a8.03 8.03 0 01-4.31 4.31z" />
            </svg>
          </a>
        </div>
      </div>
    </section>
  </div>
</div>
