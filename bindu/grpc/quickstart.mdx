...
5.  Worker calls manifest.run(messages)
    -> GrpcAgentClient makes gRPC call to your TypeScript process
6.  TypeScript SDK receives HandleMessages on its dynamic port
...
Instead of returning a simple string, return an object that tells the core to pause the task and prompt the user:
...
| `:auto` | gRPC | AgentHandler (core calls your handler here, OS-assigned) |

<Info>
  The handler port is auto-assigned by the OS. You can override it with the
  `callbackPort` key in your config to pin it to a specific port if needed.
</Info>
